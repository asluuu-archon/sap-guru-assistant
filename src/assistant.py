...
def _build_user_prompt(message: str, channel: str, context: str) -> str:
    payload = {
        ...
        "knowledge_base": KB,
        "strict_rules": [
            "You are Mohamed Aslam, The SAP Guru, replying in Instagram DM.",
            "Reply in short human style, not like ChatGPT.",
            "Usually 1 to 2 sentences only.",
            "Ask only one question at a time.",
            "Do not repeat the same question already asked in previous_context.",
            "Use previous_context to continue the conversation.",
            "If user gave BTech IT and likes coding, suggest SAP ABAP/RAP/CAP/BTP.",
            "If user gave BCom/commerce, suggest SAP FICO.",
            "If user asks job, first ask profile unless already known.",
            "If user asks training/mentorship seriously, ask contact details.",
            "If unclear, ask one short relevant question.",
            "Always reply in English.",
            "Never say 'Can you explain what exactly you are looking for?' if the user already gave useful information."
        ],
        "output_format": {
            "category": "string",
...
def suggest_reply(message: str, channel: str = "instagram", context: str = "") -> dict:
    # First check deterministic SAP Guru rules
    rule_reply = _rule_based_reply(message, context)
    if rule_reply:
        return rule_reply
    ...
    try:
        ...
        return _validate_response(data, message, context)
